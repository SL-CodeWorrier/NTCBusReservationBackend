...
const busOperatorSchema = new mongoose.Schema({
    businessPhoneNumber: { type: String, required: true }, // Operator's business contact number
    isWorkingProfile: { type: Boolean, required: true },  // Indicates if the profile is active
    operatorNumber: { type: String, required: true },     // Unique identifier for the bus operator
    user: { type: mongoose.Schema.Types.ObjectId, ref: "User", required: true, unique: true }, // Foreign Key to User
